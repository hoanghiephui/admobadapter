   ...
   */
    ...
    */
    ...
    public void onAdLoaded(int adIdx, Object adPayload) {
        ...
        setVisibilityAds((NativeExpressAdView) adPayload, View.VISIBLE);
    }
    ...
    public void onAdFailed(int adIdx, int errorCode, Object adPayload) {
        setVisibilityAds((NativeExpressAdView) adPayload, View.GONE);
        int pos = getAdapterCalculator().translateAdToWrapperPosition(Math.max(adIdx, 0));
        notifyItemRangeChanged(pos, pos + 15);
    }

    private void setVisibilityAds(NativeExpressAdView adPayload, int visibility) {
        if (adPayload != null) {
            ViewParent parent = adPayload.getParent();
            if (parent == null || parent instanceof RecyclerView)
                adPayload.setVisibility(visibility);
            else {
